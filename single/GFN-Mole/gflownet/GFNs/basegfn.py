...
class BaseTBGFlowNet():
  ...
  def batch_fwd_sample_mcmc(self, n, epsilon=0.0, uniform=False, k=4, num_chain=4):
    assert k > 0
    ...
    for _ in range(num_chain-1):
      new_complete_trajs, _ = self.backforth_sample(complete_trajs, k)
      # add data to buffer
      new_rewards = []
  ...
  def train_tb(self, batch, log = True):
    ...
    if self.args.ordering:
      batch_loss = self.ordering_batch_loss_trajectory_balance(batch)
    else:
  ...
  def ordering_batch_loss_trajectory_balance(self, batch):
    """ batch: List of [Experience].
    ...
    fwd_chain = self.batch_traj_fwd_logp(batch)
    back_chain = self.batch_traj_back_logp(batch)
    logr_chain = torch.stack([exp.logr for exp in batch])

    
    losses = self.ordering_loss(fwd_chain - back_chain + logr_chain, logr_chain)
    losses = torch.clamp(losses, max=10**2)
    mean_loss = torch.mean(losses)
    return mean_loss

  def ordering_loss(self, pred_log_rewards, log_rewards):
  ...
  def batch_traj_fwd_logf_hat_unroll(self, batch):
    trajs = [exp.traj for exp in batch]
    fwd_states, back_states, unroll_idxs = unroll_trajs(trajs)
    states_to_log_exp_F_s = []
    for state in fwd_states:
      children = self.policy_fwd.state_map_f(state)
      childs_uniq = unique_keep_order_remove_nones(children)
      _, scores_uniq, _ = self.policy_fwd.v_forward(state, childs_uniq)
      states_to_log_exp_F_s.append(torch.logsumexp(torch.exp(scores_uniq), -1))


    log_exp_F_s = torch.zeros((len(batch), self.mdp.forced_stop_len + 1)).to(device=self.args.device)
    for traj_idx, (start, end) in unroll_idxs.items():
      for i, j in enumerate(range(start, end)):
        log_exp_F_s[traj_idx][i] = states_to_log_exp_F_s[j]

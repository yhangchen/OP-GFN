...
class FMGFN(BaseTBGFlowNet):
  ...
  def train(self, batch):
    return self.train_fm(batch)

  def train_fm(self, batch, log = True):
    """ Step on detailed balance loss.
      ...
      Batching is handled in trainers.py.
    """
    batch_loss = self.batch_loss_flow_matching(batch)
    ...
    if log:
      batch_loss = tensor_to_np(batch_loss)
      print(f'FM training:', batch_loss)
      wandb.log({'Regular FM loss': batch_loss})
    return
  
  def batch_loss_flow_matching(self, batch):
    """ batch: List of [Experience].
    """
    log_F_s, _ = self.batch_traj_fwd_logp_unroll(batch)
    log_F_next_s, _ = self.batch_traj_back_logp_unroll(batch)
    
    for i, exp in enumerate(batch):
      log_F_s[i, -1] -= exp.logr.clone().detach()


    losses = (log_F_s[...,1:] - log_F_next_s[...,:-1]).pow(2).sum(axis=1)

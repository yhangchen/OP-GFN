...
class TrajectoryBalance(GFNAlgorithm):
    ...
    def compute_batch_losses(
        ...
        elif self.cfg.do_ordering:
            pred_log_R = log_Z + traj_log_p_F - traj_log_p_B
            # cosim = nn.functional.cosine_similarity(batch.flat_rewards, batch.focus_dir, dim=1)
            # log_R = torch.maximum(
            #     batch.flat_rewards.sum(dim=1).clamp(min=1e-30).log(), torch.tensor(self.global_cfg.algo.illegal_action_logreward, device=dev)
            # ).float()
            # traj_losses = self.ordering_loss(cosim, pred_log_R-log_R) + self.ordering_loss(log_R, pred_log_R)
        ...
        info = {
            "offline_loss": traj_losses.mean() if batch.num_offline > 0 else 0,
            "online_loss": traj_losses.mean() if batch.num_online > 0 else 0,
    ...
    def ordering_loss_one(self, zero_one: torch.Tensor, pred_log_rewards: torch.Tensor):
